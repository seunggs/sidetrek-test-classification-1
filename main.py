...
def load_dataset(norm1: float,
                 norm2: float, 
                 batch_size_train: int, 
                 batch_size_test: int,
                 random_seed: int) -> Tuple[torch.utils.data.dataloader.DataLoader, torch.utils.data.dataloader.DataLoader]:
...
def train(n_epochs: int,
          ...
          optimizer: Dict) -> Tuple[List[int], List[float]]:
  train_losses = []
  train_counter = []
  network.train()
...
def test(n_epochs: int,
         ...
         train_loader: torch.utils.data.dataloader.DataLoader) -> Tuple[str, List[float], List[int]]:
  test_losses = []
...
def create_training_loop(n_epochs: int,
                         network: PythonPickledFile, 
                         optimizer: Dict,
...
def save_loss(train_counter: List[int],
              ...
              test_losses: List[float]) -> FlyteFile[typing.TypeVar("png")]:
...
### Predicting digits
def predict_digits(test_loader: torch.utils.data.dataloader.DataLoader,
                   network: PythonPickledFile) -> FlyteFile[typing.TypeVar("png")]:
